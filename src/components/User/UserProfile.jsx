...
import { userDetail, userUpdateProfile, userUpdatePassword } from "../../lib/api/UserApi";
...
export default function UserProfile() {
  ...
  // function untuk mendapatkan data user sebelum, halaman dirender
  async function fetchUserDetail() {
    const response = await userDetail(token); // ambil data token dari local storage

    // membuat response body, dan menampilkannya ke console browser
    const responseBody = await response.json();
    console.log(responseBody);

    // mengecek jika response ok
    if (response.ok) {
      // maka lakukan set data state, dan simpan ke form input
  ...
  // untuk update profile (name)
  async function handleSubmitProfile(e) {
    // mencegah agar tidak reload pada saat submit
    e.preventDefault();

    // memanggil api dari lib user api
    const response = await userUpdateProfile(token, { name });
    const responseBody = await response.json();
    console.log(responseBody);

    // lakukan pengecekan jika endpoint sukses
    if (response.ok) {
      await alertSuccess("Profil berhasil diupdate");
    } else {
      alertError(responseBody.errors);
    }
  }

  // untuk update profile (password)
  async function handleSubmitPassword(e) {
    // mencegah agar tidak reload pada saat submit
    e.preventDefault();

    // melakukan pengecekan sebelum hit api
    if (password !== confirmPassword) {
      await alertError("Password tidak sama");

      // setelah muncul pesan password tidak sama, maka input field di password, dikosongi
      setPassword("");
      setConfirmPassword("");
      return;
    }
    ...
    if (response.ok) {
      ...
      // begitupun, kalau berhasil melakukan update password
      setPassword("");
      setConfirmPassword("");
    } else {
      await alertError(responseBody.errors);
  ...
  // memanggil fetch data user detail, saat pertama kali komponen di tampilkan
  useEffectOnce(() => {
    // dalam bentuk promise
    fetchUserDetail().then(() => console.log("User detail fetched succesfully"));
  });
