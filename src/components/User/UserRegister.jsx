...
export default function UserRegister() {
  // implementasi API user register

  // mendefinisikan parameter / form yang dibutuhkan
  ...
  async function handleSubmit(e) {
    // untuk mencegah agar tidak melakukan submit (karena kita pakai SPA, agar tidak reload)
    e.preventDefault();

    // melakukan pengecekan
    if (password !== confirmPassword) {
      // menampilkan alert dari lib (custom), jika password berbeda
      await alertError("Password tidak boleh berbeda");
      return;
    }

    // mengeksekusi api, jika password sama
    ...
    // setelah proses register selesai, tampilkan response body
    const responseBody = await response.json();
    console.log("Status:", response.status);
    console.log("Body:", responseBody);

    // melakukan pengecekan, jika hasil kode response sesuai (success)
    if (response.ok) {
      await alertSuccess("User berhasil di daftarkan");

      // jika berhasil, lakukan redirect. alih alih refresh, maka nanti react akan di load ulang (bukan SPA lagi)
      await navigate({
        // akan di redirect ke route login (halaman login)
        pathname: "/login",
      });
    } else {
      // kalau gagal, tampilkan pesan error dari error message
    ...
    <div className="animate-fade-in bg-gray-800 bg-opacity-80 p-8 rounded-xl shadow-custom border border-gray-700 backdrop-blur-sm w-full max-w-md">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="text-center text-sm text-gray-400">
          ...
          {/* menggunakan link untuk menuju ke halaman login */}
