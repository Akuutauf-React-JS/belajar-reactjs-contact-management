...
export default function UserLogin() {
  ...
  // menyiapkan tempat untuk token auth
  // dengan set nama key baru, dan value nya adalah kosong
  ...
  async function handleSubmit(e) {
    ...
    // mengambil respons dari user login
    const response = await userLogin({ username, password });

    // mengambil response body dan menampilkannya di console (untuk pengecekan)
    const responseBody = await response.json();
    console.log(responseBody);

    // mengecek jika response status sudah ok
    if (response.ok) {
      // sebelum melakukan redirect, token di simpan ke dalam local storage
      ...
      // kalau login gagal, maka tampilkan pesan error dari response
